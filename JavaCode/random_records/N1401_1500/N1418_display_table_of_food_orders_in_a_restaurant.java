 ...
 * time:2020/4/22
 */
public class N1418_display_table_of_food_orders_in_a_restaurant {
    ...
    public List<List<String>> displayTable(List<List<String>> orders) {
        ...
        for (List<String> order:orders)
        {
            ...
            Map<String, Integer> itemMap=map.get(id);
        ...
        List<String> itemList=res.get(0);
        for (Map.Entry<Integer,Map<String,Integer>> e:map.entrySet())
        {
            List<String> list=new ArrayList<>(1+itemList.size());
            list.add(e.getKey().toString());
            for (String item:itemList)
            {
                list.add(e.getValue().getOrDefault(item,0).toString());
            }
            res.add(list);
        }
        List<String> first=new ArrayList<>(itemList.size()+1);
        first.add("Table");
        first.addAll(itemList);
        res.set(0,first);
        return res;
    }
}
