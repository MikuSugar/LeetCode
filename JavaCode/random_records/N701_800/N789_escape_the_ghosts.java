...
public class N789_escape_the_ghosts {
    public boolean escapeGhosts(int[][] ghosts, int[] target) {
        int minGhostStep=Integer.MAX_VALUE>>1;
        for (int[] ghost:ghosts)
        {
            minGhostStep=Math.min(Math.abs(ghost[0]-target[0])+Math.abs(ghost[1]-target[1]),minGhostStep);
        }
        return Math.abs(target[0])+Math.abs(target[1])<minGhostStep;
    }
}
/**
