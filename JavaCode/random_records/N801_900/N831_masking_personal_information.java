 ...
 * time:2019/9/19
 */
public class N831_masking_personal_information {
    public String maskPII(String S) {
        if(S.contains("@"))return sloveMail(S);
        return slovePhone(S);
    }

    private String slovePhone(String s) {
        ...
        if(sb.length()==10) return "***-***-"+last;
        if(sb.length()==11) return "+*-***-***-"+last;
        if(sb.length()==12) return "+**-***-***-"+last;
        return "+***-***-***-"+last;
    }

    private String sloveMail(String s) {
        String[] str=s.split("@");
        String pre=str[0].toLowerCase();
        String last=str[1].toLowerCase();
        return pre.charAt(0)+"*****"+pre.charAt(pre.length()-1)+"@"+last;
    }
}
/**
