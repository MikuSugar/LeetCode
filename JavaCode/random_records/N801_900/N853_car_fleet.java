...
public class N853_car_fleet {
    public int carFleet(int target, int[] position, int[] speed) {
        ...
        Arrays.sort(car,(o1, o2)->o2[0]-o1[0]);
        int res=1;
        double time=(target*1.0-car[0][0])/car[0][1];
        double end=target;
        for (int i=1;i<car.length;i++)
        {
            double temp=car[i][0]+car[i][1]*time;
            if(temp<end)
            {
                res++;
                time=(target*1.0-car[i][0])/car[i][1];
            }
        }
        return res;
    }
