 ...
 * @author mikusugar
 */
object N1984_minimum_difference_between_highest_and_lowest_of_k_scores {
  def main(args: Array[String]): Unit = {
    println(minimumDifference(Array(1, 2, 3, 6), 4))
  }

  def minimumDifference(nums: Array[Int], k: Int): Int = {
    val sortNums = nums.sorted
    (0 to (sortNums.length - k)).map(idx => sortNums(idx + k - 1) - sortNums(idx)).min
  }
}
