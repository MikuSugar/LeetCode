 ...
 * @author mikusugar
 */
object N1725_number_of_rectangles_that_can_form_the_largest_square {
  def countGoodRectangles(rectangles: Array[Array[Int]]): Int = {
    val line = rectangles.map(arr => Math.min(arr(0), arr(1)))
    val maxLine = line.max
    line.count(_ == maxLine)
  }
}
