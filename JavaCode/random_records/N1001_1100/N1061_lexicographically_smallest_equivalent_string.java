 ...
 * time:2019/11/19
 */
public class N1061_lexicographically_smallest_equivalent_string {
    public String smallestEquivalentString(String A, String B, String S) {
        char[] a=A.toCharArray();
        char[] b=B.toCharArray();
        int[] fa=new int[26];
        for (int i=0;i<26;i++)fa[i]=i;
        for (int i=0;i<a.length;i++) union(a[i],b[i],fa);

        StringBuilder res=new StringBuilder();
        for (char c:S.toCharArray()) res.append((char) (find(c-'a',fa)+'a'));
        return res.toString();
    }

    private void union(char a, char b, int[] fa) {
        int af=find(a-'a',fa);
        int bf=find(b-'a',fa);
        int res=Math.min(af,bf);
        fa[af]=res;
        fa[bf]=res;
    }

    private int find(int i , int[] fa) {
        return fa[i]==i?i:find(fa[i],fa);
    }
}
/*
